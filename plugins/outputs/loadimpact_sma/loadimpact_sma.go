...
func (f *LoadimpactSma) SetSerializer(serializer serializers.Serializer) {
	f.serializer = serializer
}

func (f *LoadimpactSma) Connect() error {
	writers := []io.Writer{}
...
func (f *LoadimpactSma) Close() error {
	var errS string
...
func (f *LoadimpactSma) SampleConfig() string {
	return sampleConfig
}

func (f *LoadimpactSma) Description() string {
	return "Send telegraf metrics to file(s)"
}

func (f *LoadimpactSma) Write(metrics []telegraf.Metric) error {
	if len(metrics) == 0 {
		return nil
	}
